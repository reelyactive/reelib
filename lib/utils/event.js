...
function isFiltered(event, criteria) {
  ...
  if(criteria.hasOwnProperty('deviceIds') &&
     criteria.deviceIds instanceof Array) {
    if(criteria.deviceIds.indexOf(flattenedEvent.deviceId) >= 0) {
      criteriaMet = true;
    }
    for(var cId = 0; cId < flattenedEvent.deviceAssociationIds.length; cId++) {
      var id = flattenedEvent.deviceAssociationIds[cId];
      if(criteria.deviceIds.indexOf(id) >= 0) {
        criteriaMet = true;
      }
    }
    if(!criteriaMet) {
  ...
  if(criteria.hasOwnProperty('receiverIds') &&
     criteria.receiverIds instanceof Array) {
    if(criteria.receiverIds.indexOf(flattenedEvent.receiverId) < 0) {
      return false;
    }
    criteriaMet = true;
  }

  if(criteria.hasOwnProperty('rssiRange')) {
    if(criteria.rssiRange.hasOwnProperty('minimum') &&
       (flattenedEvent.rssi < criteria.rssiRange.minimum)) {
      return false;
    }
    if(criteria.rssiRange.hasOwnProperty('maximum') &&
       (flattenedEvent.rssi > criteria.rssiRange.maximum)) {
      return false;
    }
    criteriaMet = true;
  }
