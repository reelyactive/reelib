...
var IBEACON_BITS = 160;
var UUID128_NIBBLES = UUID128_BITS / BITS_PER_NIBBLE;
var IBEACON_NIBBLES = IBEACON_BITS / BITS_PER_NIBBLE;
...
/**
 * Return whether there's at least one match between the two given identifier
 * strings or identifier string arrays.
 * @param {Object} identifiers1 The first identifier (String or Array).
 * @param {Object} identifiers2 The second identifier (String or Array).
 * @return {boolean} True if the given identifier is valid, false otherwise.
 */
function isMatch(identifiers1, identifiers2) {
  if(!(identifiers1 instanceof Array)) {
    return isMatch([identifiers1], identifiers2);
  }
  if(!(identifiers2 instanceof Array)) {
    return isMatch(identifiers1, [identifiers2]);
  }

  for(var cId = 0; cId < identifiers1.length; cId++) {
    var id = identifiers1[cId];
    if(identifiers2.indexOf(id) >= 0) {
      return true;
    }
    if((id.length === IBEACON_NIBBLES) &&
       (identifiers2.indexOf(id.substr(0, UUID128_NIBBLES)) >= 0)) {
      return true;
    }
  }

  for(var cId = 0; cId < identifiers2.length; cId++) {
    var id = identifiers2[cId];
    if((id.length === IBEACON_NIBBLES) &&
       (identifiers1.indexOf(id.substr(0, UUID128_NIBBLES)) >= 0)) {
      return true;
    }
  }
