...
function getAssociationIds(tiraid) {
  ...
  if(tiraid && tiraid.hasOwnProperty('identifier')) {
    ...
    // Organised in order of likelihood of being a unique device identifier,
    // a specific iBeacon having high likelihood, a company identifier having
    // a low likelihood.
    var advDataExists = tiraid.identifier.hasOwnProperty("advData");
    if(advDataExists) {
      var advData = tiraid.identifier.advData;

      // LE Bluetooth Address
      if(advData.leBluetoothDeviceAddress) {
        associationIds.push(advData.leBluetoothDeviceAddress);
      }
      // Eddystone-UID
      if(advData.serviceData && advData.serviceData.eddystone &&
         advData.serviceData.eddystone.uid) {
        associationIds.push(advData.serviceData.eddystone.uid.namespace +
                            advData.serviceData.eddystone.uid.instance);
      }
      // iBeacon
      ...
      // Estimote 64-bit identifier
      if(advData.manufacturerSpecificData &&
         advData.manufacturerSpecificData.nearable) {
        associationIds.push(advData.manufacturerSpecificData.nearable.id);
      }
      if(advData.serviceData && advData.serviceData.estimote) {
        associationIds.push(advData.serviceData.estimote.id);
      }
      // Eddystone-EID
      if(advData.serviceData && advData.serviceData.eddystone &&
         advData.serviceData.eddystone.eid) {
        associationIds.push(advData.serviceData.eddystone.eid);
      }
    ...
    // Proprietary EUI-64
    if(tiraid.identifier.value && tiraid.identifier.type &&
       (tiraid.identifier.type === identifier.EUI64)) {
      var oui36 = tiraid.identifier.value.substr(0, 9); // TODO: create func?
      associationIds.push(oui36);
    }
...
function toFlattened(tiraid) {
  ...
  flattened.deviceAssociationIds = getAssociationIds(tiraid);
